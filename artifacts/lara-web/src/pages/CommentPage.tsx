...
const DEFAULT_COMMENTS = ["🔥🔥🔥", "❤️❤️❤️", "Wow amazing!", "So cute!", "Love this!", "Grabe! 😍", "Sana all 😂", "Ikaw na! 💯"];

export default function CommentPage({ profile, onBack }: Props) {
  const [postUrl, setPostUrl] = useState("");
  const [commentText, setCommentText] = useState(DEFAULT_COMMENTS.join("\n"));
  ...
  async function handleComment() {
    ...
    if (!comments.length) { showToast("Enter at least one comment", "error"); return; }
    setLoading(true);
    setLogs([]);
    setResult(null);
    try {
      const res = await api.comment(profile.cookie, postUrl.trim(), comments, count);
      setLogs(res.logs || []);
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "0 0 32px" }}>
      <Header title="Mass Comment" emoji="💬" onBack={onBack} />
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <label style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--lara-muted)", marginBottom: 8, fontWeight: 600 }}>
            ...
            <span style={{ color: "#a855f7" }}>{commentText.split("\n").filter(s => s.trim()).length} comments</span>
          </label>
          <textarea
        ...
        <button className="lara-btn lara-btn-primary" style={{ marginTop: 20 }} onClick={handleComment} disabled={loading}>
          {loading ? <Spinner /> : <>💬 Comment {count} Times</>}
        </button>
      </div>
    </div>
  );
